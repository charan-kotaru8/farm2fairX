...
export default function LoginPage() {
  ...
  const { user, profile, login, loginWithGoogle } = useAuth();
  ...
  // Auto-redirect if already logged in (e.g. after Google OAuth)
  useEffect(() => {
    if (user) {
  ...
  return (
    <div className="container mx-auto px-4 py-16 flex justify-center">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-border w-full max-w-md">
        ...
        <p className="text-sm text-center text-muted-foreground mb-6">Sign in to your Farm2Fair account</p>
        ...
        <form onSubmit={handleLogin} className="flex flex-col gap-4">
          <div>
            ...
            <input 
              ...
              placeholder="farmer@farm2fair.com" 
            />
          </div>
          <div>
        ...
        <div className="mt-6 text-center text-sm text-muted-foreground">
          Don't have an account? <a href="/signup" className="text-secondary hover:underline font-medium">Sign up</a>
        </div>
      </div>
    </div>
  );
